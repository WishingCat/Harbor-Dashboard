...
/** Mint an API key in the browser so an Agent needs no login credentials. */
function TokenManager({projects, user, origin, onLogin, notify}: {projects: Project[]; user: User | null; origin: string; onLogin: () => void; notify: (message: string) => void}) {
  ...
  // A key minted before keys covered the whole account still shows its project.
  const scopeLabel = (id: string | null) => id === null ? '全部项目' : projects.find(item => item.id === id)?.name || id;
  ...
  async function create() {
    if (busy) return;
    if (!name.trim()) {setError('请填写 Key 名称，便于以后识别和撤销。'); return;}
    setBusy(true); setError('');
    try {
      const result = await post<{token: string; api_token: ApiToken}>('/auth/tokens', {name: name.trim(), expires_in_days: expiry});
  ...
  if (!user) return <div className="token-panel"><p>登录后可以直接生成 API Key，无需在终端里输入账号密码。</p><button type="button" className="button primary" onClick={onLogin}><KeyRound size={15} />登录并生成 API Key</button></div>;
  return <div className="token-panel">
    <div className="token-form">
      <label>Key 名称<input value={name} maxLength={80} disabled={busy} onChange={event => setName(event.target.value)} placeholder="例如 Agent upload" /></label>
      ...
      <button type="button" className="button primary" disabled={busy} onClick={() => void create()}>{busy ? <LoaderCircle size={15} className="spin" /> : <KeyRound size={15} />}生成 API Key</button>
    </div>
    <p className="api-guide-note">一把 Key 适用于你能访问的<strong>全部项目</strong>，不必为每个项目各生成一把。上传时用 <code>project_id</code> 和 <code>task_set_id</code> 指定目标；下方选好项目与任务集后，示例命令会自动填好这两个值。Key 代表你的账号，请勿共享；不再需要时随时撤销。</p>
    {error && <p className="form-error" role="alert">{error}</p>}
    {secret && <div className="token-reveal">
      <div className="token-reveal-heading"><KeyRound size={15} /><strong>API Key 已生成，只显示这一次</strong></div>
    ...
    {!loaded ? <p className="api-guide-note">正在读取已有 API Key…</p>
      : tokens.length === 0 ? <p className="api-guide-note">还没有生成过 Token。</p>
      : <div className="api-guide-table-wrap"><table className="token-table"><thead><tr><th>名称</th><th>适用范围</th><th>状态</th><th>到期</th><th>最近使用</th><th aria-label="撤销" /></tr></thead><tbody>{tokens.map(token => {
          const state = tokenState(token);
          return <tr key={token.id}><td>{token.name}</td><td>{scopeLabel(token.project_id)}</td><td><span className={`token-state ${state.className}`}>{state.label}</span></td><td>{shortDate(token.expires_at)}</td><td>{token.last_used_at ? shortDate(token.last_used_at) : '未使用'}</td><td>{!token.revoked && <button type="button" className={`token-revoke ${confirming === token.id ? 'armed' : ''}`} aria-label={confirming === token.id ? `确认撤销 ${token.name}` : `撤销 ${token.name}`} title="撤销后使用它的 Agent 立即失效" disabled={busy} onClick={() => void revoke(token)}>{confirming === token.id ? '确认撤销' : <Trash2 size={15} />}</button>}</td></tr>;
        })}</tbody></table></div>}
  </div>;
}

const endpoints = [
  ['GET', '/api/v1/projects', '本 Key 可用的项目', '—'],
  ['GET', '/api/v1/task-sets', '列出任务集', 'project_id'],
  ['POST', '/api/v1/task-sets', '新建任务集', 'name、project_id'],
  ['GET', '/api/v1/tags', '预设标签词表与已用标签', 'project_id'],
  ['GET', '/api/v1/tasks', '列出任务，可按任务集与标签筛选', 'project_id、task_set_id、tag'],
  ['POST', '/api/v1/tasks', '上传任务及包内已有结果', 'files、project_id、task_set_id、description、tags'],
  ['GET', '/api/v1/tasks/{task_id}', '任务、文件、结果与评审（含 can_delete）', 'project_id'],
  ['POST', '/api/v1/tasks/{task_id}/rollouts', '追加已有产物', 'files、project_id'],
  ['DELETE', '/api/v1/tasks/{task_id}', '删除任务，仅作者或管理员', 'project_id'],
  ['GET', '/api/tasks/{task_id}/archive', '打包下载文件（公开，无需 Key）', 'project_id、prefix、rollout_id'],
];
...
export default function ApiGuidePage({project, projects, taskSets, user, onProjectChange, onLogin, notify}: {project: Project; projects: Project[]; taskSets: TaskSet[]; user: User | null; onProjectChange: (projectId: string) => void; onLogin: () => void; notify: (message: string) => void}) {
  ...
  // No project_id: the key covers every project, and each call names its own target.
  const tokenBody = shellQuote(JSON.stringify({name: 'Agent upload', expires_in_days: 30}));
...
curl --fail-with-body --show-error --get \\
  ...
  const pythonDelete = `${followupGuard} &&
python3 harbor_upload.py delete "$HARBOR_TASK_ID" \\
  --project "$HARBOR_PROJECT_ID" --task-set "$HARBOR_TASK_SET_ID"`;
  const curlDelete = `${followupGuard} &&
curl --fail-with-body --show-error -X DELETE \\
  "$HARBOR_API_URL/api/v1/tasks/$HARBOR_TASK_ID?project_id=$HARBOR_PROJECT_ID"`;
...
curl --fail-with-body --show-error --get \\
  ...
  return <div className="page-content api-guide-page">
    ...
    <div className="api-guide-content">
      <p className="api-guide-intro">上传 Harbor 任务与已有产物，获取团队质检结果。平台保存文件，不执行 Rollout。本页包含 Agent 接入所需的全部信息：先生成 API Key，再选择上传目标，下方命令可直接复制运行。</p>
      <section className="api-guide-keyblock" aria-labelledby="api-guide-key">
        <div className="api-guide-section-heading"><h2 id="api-guide-key"><KeyRound size={18} />生成 API Key</h2></div>
        <TokenManager projects={projects} user={user} origin={origin} onLogin={onLogin} notify={notify} />
        <details className="api-guide-nested"><summary>没有浏览器时：在终端里生成<ChevronDown size={16} /></summary><p>下面的命令通过登录 Cookie 创建一把 30 天 Key，并读入 <code>HARBOR_API_TOKEN</code>；账号与密码输入不会回显。</p><CodeExample title="登录并创建 API Key" code={authentication} notify={notify} /></details>
      </section>
      ...
      <details className="api-guide-disclosure">
        <summary><span>追加产物与查询质检</span><ChevronDown size={17} /></summary>
        <div className="api-guide-disclosure-body"><p>沿用上一步的环境变量，将已有产物追加到该任务；仅任务作者或管理员可追加。只查询时运行下面最后一条命令。</p><CodeExample title={method === 'python' ? '追加已有结果并查询' : '追加结果 ZIP 并查询'} code={method === 'python' ? pythonFollowup : curlFollowup} notify={notify} /><dl className="api-guide-status"><div><dt><code>task.status</code></dt><dd><code>pending</code> 待评审 · <code>approved</code> 通过 · <code>changes_requested</code> 需修改</dd></div><div><dt><code>reviews</code></dt><dd>评审者的意见与结论。</dd></div><div><dt><code>rollouts[].reward</code></dt><dd>已有运行的 Reward，与人工质检状态分别保存。</dd></div></dl></div>
      </details>

      <details className="api-guide-disclosure">
        <summary><span>删除任务</span><ChevronDown size={17} /></summary>
        <div className="api-guide-disclosure-body"><p>仅任务作者或管理员可以删除。任务详情接口返回的 <code>can_delete</code> 说明当前 Key 是否有权删除某道任务。</p><CodeExample title={method === 'python' ? '删除任务' : '删除任务（curl）'} code={method === 'python' ? pythonDelete : curlDelete} notify={notify} /><p className="api-guide-note"><strong>删除不可撤销</strong>，会一并删除任务文件、全部 Rollout 结果、<strong>其他人提交的评审</strong>以及活动记录。删除后使用原 <code>Idempotency-Key</code> 重新上传会正常创建新任务，不会返回已删除任务的旧响应。网页端在任务页右上角也有「删除任务」按钮。</p></div>
      </details>
      ...
      <details className="api-guide-disclosure">
        ...
        <div className="api-guide-disclosure-body"><p>以下接口均需 <code>Authorization: Bearer $HARBOR_API_TOKEN</code>。API Key 适用于你能访问的全部项目，因此 <strong><code>project_id</code> 必填</strong>；省略会返回 <code>422</code> 并列出可用项目。上传与追加通过 multipart 字段传 <code>project_id</code> / <code>task_set_id</code>，列表、详情与删除通过查询参数传。</p><div className="api-guide-table-wrap"><table><thead><tr><th>方法</th><th>路径</th><th>用途</th><th>主要参数</th></tr></thead><tbody>{endpoints.map(([methodName, path, usage, params]) => <tr key={`${methodName}${path}`}><td>{methodName}</td><td><code>{path}</code></td><td>{usage}</td><td>{params}</td></tr>)}</tbody></table></div><ul><li>新建任务集使用 JSON：<code>name</code> 为 1–80 字符，<code>project_id</code> 可选。响应为 <code>{'{task_set: {...}}'}</code>；列表返回 <code>task_sets</code>。</li><li>上传使用 multipart：必填 <code>files</code>，可填 <code>project_id</code>、<code>task_set_id</code>。任务支持 <code>description</code>；结果支持 <code>name</code>、<code>agent</code>、<code>model</code>。</li><li>直接上传多个文件时，重复 <code>files</code>，并用 <code>paths</code> 传对应顺序的 JSON 相对路径数组。</li><li>任务列表与详情使用查询参数 <code>task_set_id</code>。上传时省略则进入该项目的默认任务集；详情、追加与删除按任务实际归属识别。</li>
                <li><code>tags</code> 为 JSON 数组字符串，最多 20 项、每项 ≤50 字符。预设词表见 <code>GET /api/v1/tags</code>，也接受任何自定义标签；省略该字段则沿用任务包 <code>task.toml</code> 中声明的标签。按标签查询时重复 <code>tag</code> 参数取交集。</li>
                <li><code>GET /api/v1/tasks/{'{task_id}'}</code> 返回 <code>task</code>、<code>files</code>、<code>rollouts</code>、<code>reviews</code> 和 <code>can_delete</code>；上传与追加额外返回 <code>task_url</code>、<code>api_url</code> 和 <code>replayed</code>。</li>
                <li>打包下载 <code>GET /api/tasks/{'{task_id}'}/archive</code> 返回 ZIP：<code>prefix</code> 指定文件夹（省略则打包全部），<code>rollout_id</code> 打包某个 Rollout 的文件。与单文件下载一样属于公开阅读接口，不需要 Bearer Key。</li></ul><div className="api-guide-table-wrap"><table><thead><tr><th>状态码</th><th>处理方式</th></tr></thead><tbody><tr><td>400 / 422</td><td>检查文件、字段或任务结构；缺少 <code>project_id</code> 也返回 422。</td></tr><tr><td>401</td><td>Token 无效、已撤销或过期，重新获取。</td></tr><tr><td>403 / 404</td><td>检查项目、任务集归属，以及追加或删除的权限。</td></tr><tr><td>409</td><td>同一幂等标识对应了不同内容。</td></tr><tr><td>413</td><td>检查上传大小或文件数量。</td></tr><tr><td>5xx / 断连</td><td>上传保留原标识重试；新建任务集先检查列表。</td></tr></tbody></table></div><p className="api-guide-note">Key 元数据：用登录 Cookie 调用 <code>GET /api/auth/tokens</code>；撤销使用 <code>DELETE /api/auth/tokens/{'{token_id}'}</code>。这两个接口需要浏览器会话，Bearer Key 无法调用，也无法用一把 Key 生成另一把。</p></div>
      </details>
    </div>
  </div>;
}
