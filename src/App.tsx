...
import { ArrowDown, ArrowDownUp, ArrowLeft, ArrowUpRight, BookOpen, ChevronDown, ChevronRight, CircleCheck, CircleDashed, ExternalLink, FolderClosed, HelpCircle, LayoutGrid, ListFilter, LogIn, LogOut, Menu, Moon, Play, Plus, Search, Settings2, SquareTerminal, Sun, Users, X } from 'lucide-react';
...
export default function App() {
  ...
  const matchedTasks = scopedTasks.filter(t => `${t.title} ${t.description} ${t.tags.join(' ')} ${t.author}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <div className={`app-shell ${taskId ? 'reading-view' : ''} ${navigationDocked ? 'navigation-pinned' : ''}`}>
    ...
    <div className="workspace-main"><header className="topbar"><div className="breadcrumb">
      ...
      <main>{error ? <div className="page-content"><Empty title="暂时无法连接工作区" description={error} action={<button className="button primary" onClick={() => {setBusy(true); void refresh();}}>重新连接</button>} /></div>
        ...
        : page === 'api' ? <ApiGuidePage key={project.id} project={project} taskSets={taskSets} notify={notify} />
    ...
    {helpOpen && <Modal title="使用指南" onClose={() => setHelpOpen(false)}><div className="guide-content"><div><span>01</span><section><h3>上传任务</h3><p>先新建或选择项目和任务集，再上传含 instruction.md 和 task.toml 的 ZIP 或任务文件夹。任务自动命名，简介可选。</p></section></div><div><span>02</span><section><h3>查看已有结果</h3><p>上传包中包含 Rollout 结果时，可查看 result.json、Agent 轨迹和产物文件。</p></section></div><div><span>03</span><section><h3>阅读、翻译与评审</h3><p>选择文件阅读，开启双语翻译。登录后可提出修改建议，并提交“通过”或“需修改”的评审。</p></section></div><a className="button secondary" href="https://www.harborframework.com/docs" target="_blank" rel="noreferrer">Harbor 官方文档<ExternalLink size={14} /></a><p className="form-footnote">带“临时”标记的任务用于试用。首次注册的账号为管理员。翻译由平台统一提供，登录后即可使用。</p></div></Modal>}
...
function TaskLibrary({title, onBack, tasks, allTasks, query, onQuery, onOpen, onUpload, searchRef}: {title?: string; onBack?: () => void; tasks: Task[]; allTasks: Task[]; query: string; onQuery: (v: string) => void; onOpen: (id: string) => void; onUpload?: () => void; searchRef: RefObject<HTMLInputElement|null>}) {
  ...
  const filtered = useMemo(() => tasks.filter(t => (status === 'all' || t.status === status) && (category === 'all' || t.category === category) && (difficulty === 'all' || t.difficulty === difficulty) && (showDemos || !t.is_demo)).sort((a, b) => (new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()) * (oldest ? -1 : 1)), [tasks, status, category, oldest, difficulty, showDemos]);
