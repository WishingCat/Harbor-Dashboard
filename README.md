...
- 任务说明：任务包内的 `任务说明.md` 会被读取，其一级标题作为任务名、`## 任务摘要` 一段作为任务库列表里的一两句说明；没有该文件时沿用自动名称、不显示说明。
...
**平台不审查包内格式。**上传的 ZIP 或文件夹不必是标准 Harbor 任务，内容会原样保存并以文件形式展示。包内有唯一的 `task.toml` 时，会从中读取任务名称、分类、难度和标签；没有或解析失败时使用默认值，任务名取 ZIP 文件名或目录名。包内若有 `任务说明.md`，其一级标题会覆盖自动名称、`## 任务摘要` 一段会作为任务说明展示（任务库列表与详情页均显示，列表最多两行）。解析失败或没有该文件时静默沿用上面的默认值，不影响上传；`slug` 始终由自动名称派生，与标题改动无关。包内有可识别的 `result.json` 时，对应文件会作为 Rollout 结果展示。

仍然生效的是安全与容量限制：拒绝越界路径、绝对路径、符号链接和加密 ZIP，单次上传 50 MB、展开后 100 MB、单文件 20 MB、最多 2,000 个文件。
