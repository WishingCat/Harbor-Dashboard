...
**平台不审查包内格式。**上传的 ZIP 或文件夹不必是标准 Harbor 任务，内容会原样保存并以文件形式展示。包内有唯一的 `task.toml` 时，会从中读取任务名称、分类、难度和标签；没有或解析失败时使用默认值，任务名取 ZIP 文件名或目录名。包内有可识别的 `result.json` 时，对应文件会作为 Rollout 结果展示。
